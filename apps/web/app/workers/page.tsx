...
import { authenticatedHeaders, currentOrganization } from "@/lib/auth-headers";
...
export default function WorkersPage() {
  ...
  async function loadWorkers() { const r = await fetch("/api/backend/workers", { headers: await authenticatedHeaders() }); if (r.ok) setWorkers(await r.json()); }
  useEffect(() => { void loadWorkers(); }, []);
  async function submit(event: FormEvent) { event.preventDefault(); setError(""); const organization = await currentOrganization(); const r = await fetch("/api/backend/workers", { method: "POST", headers: await authenticatedHeaders({ "content-type": "application/json" }), body: JSON.stringify({ organization_id: organization, employee_number: employeeNumber, full_name: name }) }); if (!r.ok) { setError("Worker belum berhasil ditambahkan."); return; } setName(""); setEmployeeNumber(""); await loadWorkers(); }
